...
export function AgentActions({ actions, isProcessing = false, variant = 'card' }: AgentActionsProps) {
  if (actions.length === 0 && isProcessing) {
    const inner = (
      <div className="flex items-center gap-2 py-4 justify-center">
        <div className="animate-spin h-4 w-4 border-2 border-gray-400 border-t-transparent rounded-full" />
        <p className="text-sm text-gray-500">Agent is processing…</p>
      </div>
    );
    if (variant === 'inline') return inner;
    return (
      <div className="bg-white border border-gray-200 rounded-xl p-5 shadow-sm">
        <h3 className="text-sm font-semibold text-gray-500 uppercase tracking-wide mb-3">Agent activity</h3>
        {inner}
      </div>
    );
  }
  ...
  if (actions.length === 0) {
    const inner = <p className="text-sm text-gray-400 text-center py-4">No agent actions yet.</p>;
    if (variant === 'inline') return inner;
    return (
      <div className="bg-white border border-gray-200 rounded-xl p-5 shadow-sm">
        <h3 className="text-sm font-semibold text-gray-500 uppercase tracking-wide mb-3">Agent activity</h3>
        ...
        return (
          <div key={action.id} className="flex gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className={`text-xs text-gray-500 mt-0.5 ${variant === 'inline' ? 'line-clamp-1' : 'line-clamp-2'}`}>
                {action.rationale}
              </p>
              <div className="flex items-center gap-3 mt-1 text-xs text-gray-400">
                {variant === 'inline' ? (
                  <>
                    {action.amount !== '0' && action.expectedApyAfter != null && (
                      <span>${parseFloat(action.amount).toLocaleString()} {action.asset} · {action.expectedApyAfter.toFixed(2)}% APY</span>
                    )}
                    {action.amount !== '0' && action.expectedApyAfter == null && (
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-xl p-5 shadow-sm">
      <h3 className="text-sm font-semibold text-gray-500 uppercase tracking-wide mb-4">Agent activity</h3>
      {list}
    </div>
  );
}
